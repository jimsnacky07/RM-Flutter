                                ...
                                @foreach ($pesanans as $index => $pesanan)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @endphp
                                            <span
                                                class="badge bg-{{ $statusColors[$pesanan->status ?? 'Menunggu Pembayaran'] ?? 'secondary' }}">
                                                <i
                                                    class="fas fa-{{ $statusIcons[$pesanan->status ?? 'Menunggu Pembayaran'] ?? 'question' }} me-1"></i>
                                                {{ $pesanan->status == 'Menunggu Pembayaran' ? 'Menunggu Konfirmasi' : $pesanan->status ?? 'Menunggu Pembayaran' }}
                                            </span>
                                        </td>
                                        <td>
...
@foreach ($pesanans as $pesanan)
    <div class="modal fade" id="updateStatusModal{{ $pesanan->id }}" tabindex="-1"
        aria-labelledby="updateStatusModalLabel{{ $pesanan->id }}" aria-hidden="true">
                ...
                <form action="{{ route('operator.pesanan.updateStatus', $pesanan->id) }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="status{{ $pesanan->id }}" name="status" required>
                                <option value="Menunggu Pembayaran"
                                    {{ $pesanan->status == 'Menunggu Pembayaran' ? 'selected' : '' }}>Menunggu
                                    Pembayaran</option>
                        ...
                        <div class="alert alert-info">
                            ...
                            {{ $pesanan->user->name ?? ($pesanan->nama_pelanggan ?? 'Pelanggan') }}<br>
